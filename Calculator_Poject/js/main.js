// creates an object ot keep track of values
const Calculator = {
...
function Input_Decimal(dot) {
    ...
    if (!Calculator.Display_Value.includes(dot)) {
        // we are saying that if the Display_Value does not contain a decimal pouint
...
function Handle_Operator(Next_Operator) {
    ...
    if (First_Operand == null) {
        ...
        // here we add a fixed amount of numbers afterthe decimal
...
keys.addEventListener('click', (event) => {
    // the target variable is an object that represents the elements
    // that was clicked
    const { target } = event;
